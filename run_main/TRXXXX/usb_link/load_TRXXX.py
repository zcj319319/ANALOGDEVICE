...
# from ClkOptDialog import ClkOptDialog
# from ControlSPI_FT4232 import spi_attribute
# from mainDialog1 import MainDialog1
...
from run_main.TRXXXX.ClkOptDialog import ClkOptDialog
from run_main.TRXXXX.usb_link.ControlSPI_FT4232 import spi_attribute
from run_main.TRXXXX.usb_link.TR_UI import Ui_MainWindow
from run_main.TRXXXX.mainDialog1 import MainDialog1


class TRPanel_operation(QtWidgets.QMainWindow, Ui_MainWindow):
    def __init__(self):
        super().__init__()
        self.setupUi(self)
    ...
    def init_spi_config(self):
        ...
        # Scan device
        if len(dev_urls) == 0:
            self.label_6.setPixmap(QtGui.QPixmap(":/images/OFF.png"))
        else:
            self.label_6.setPixmap(QtGui.QPixmap(":/images/on.png"))
    ...
    def write_atom(self, addr, data, flag=None):
        ...
        try:
            ...
            # else:
            #     new_addr = addr_str
            addr_str = '{:0>6x}'.format(addr * 8)
            data_str = '{:0>8x}'.format(data)
            write_buffer.append(int(addr_str[0:2], 16))
            write_buffer.append(int(addr_str[2:4], 16))
            write_buffer.append(int(addr_str[4:], 16))
            write_buffer.append(int(data_str[0:2], 16))
            write_buffer.append(int(data_str[2:4], 16))
            write_buffer.append(int(data_str[4:6], 16))
            write_buffer.append(int(data_str[6:], 16))
            ...
            self.spi_a.write(write_buffer, 4)
            if flag is None:
                self.textBrowser_normal_log('write addr = ' + addr_str_1 + ' data = ' + data_str_1)
        except Exception as e:
    ...
    def read_atom(self, addr, flag=None):
        ...
        try:
            ...
            # write_buffer.append(int(new_addr[2:], 16))
            addr_str_n = '{:0>6x}'.format(addr * 8)
            write_buffer.append(int(addr_str_n[0:2], 16) + 128)
            write_buffer.append(int(addr_str_n[2:4], 16))
            write_buffer.append(int(addr_str_n[4:6], 16))
            read_data = self.spi_a.exchange(write_buffer, 4)
            print(read_data)

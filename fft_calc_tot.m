...
if imd_mode==1
    ...
    performance.index_SIG2=index_SIG2;
...
if imd_mode==1
    ...
    for n=2:num_IMD
        for k=1:2*(n-1)
            ...
            if ~isempty(intersect(by,index_center_collect))  % if center collide, then disable
                disable_IMD(n-1,k)=1;
                performance.disable_IMD=disable_IMD;
            else
                disable_IMD(n-1,k)=0;
                performance.disable_IMD=disable_IMD;
            end
...
if num_interleave~=1
    ...
    for n=1:2*length(index_center_IL_OS)  % only select IL tones around <= fs/2
        ...
        performance.index_center_IL_GTS_SIG1=index_center_IL_GTS_SIG1;
        performance.disable_IL_GTS_SIG1=disable_IL_GTS_SIG1;
    end
    if imd_mode==1
        %sig2
        % gain and skew
        for n=1:2*length(index_center_IL_OS)  % only select IL tones around <= fs/2
            ...
            if ~isempty(intersect(by,index_center_collect))
                ...
                performance.disable_IL_GTS_SIG2=disable_IL_GTS_SIG2;
            else
            ...
            index_center_collect=[index_center_collect index_center_IL_GTS_SIG2(n)];
             performance.index_center_IL_GTS_SIG2=index_center_IL_GTS_SIG2;
        end
    end
end
...
if refclk_ratio~=1
    ...
    for n=1:refclk_ratio  % only select spur around <= fs/2
        ...
        performance.index_center_REF_SPUR_SIG1=index_center_REF_SPUR_SIG1;
        if ~isempty(intersect(by,index_center_collect))
            disable_REF_SPUR_SIG1(n)=1;
            performance.disable_REF_SPUR_SIG1=disable_REF_SPUR_SIG1;
        else
    ...
    if imd_mode==1
        ...
        for n=1:refclk_ratio  % only select IL tones around <= fs/2
            ...
            performance.index_center_REF_SPUR_SIG2=index_center_REF_SPUR_SIG2;
            performance.disable_REF_SPUR_SIG2=disable_REF_SPUR_SIG2;
        end
    end
end
...
res_index=setdiff([1:(N_nq/2+1)],index_collect_sfdr);
[dummy,bx]=max(mag_all(res_index));  % bx is max index in res_index it self
bx=res_index(bx);                    % so get orignal index in res_index
index_center_spur=bx;
...
if isfield(para,'fdet_array')
    for n=1:length(para.fdet_array)
        ...
        ftest=mod(fdet,fs); % fold to 0~fs
        by=floor(ftest/fs*N_nq)+1; % change to by
        if(by>N_nq/2)
            by=N_nq-(by-1)+1;   % same issue, by-1 change to 0first, after fold, then +1 change to 1first
        end
...
performance.num_IMD=num_IMD;
performance.plot_range=plot_range;
performance.dacOSR=dacOSR;
performance.N_nq=N_nq;
performance.N_fft=N_fft;
performance.fs=fs;
performance.nq=nq;
performance.mag_full=mag_full;
performance.mag_all=mag_all;
performance.index_SIG1=index_SIG1;
%% plot
if plot_option==1
    ...
    plot(sig_ind,sig_yrange,'b');
    performance.sig_ind=sig_ind;
    performance.sig_yrange=sig_yrange;
    if imd_mode==1
        performance.imd_mode=imd_mode;
        imd_mode_ind=f_trans((index_SIG2-1),N_fft,fs,nq);
        imd_mode_yrange=20*log10(mag_all(f_flip(index_SIG2,nq)));
        performance.imd_mode_ind =imd_mode_ind;
        performance.imd_mode_yrange= imd_mode_yrange;
        plot(imd_mode_ind,imd_mode_yrange,'b');
    end
    
    if isfield(para,'sigbw')
        performance.sigbw=sigbw;
        plot([sigbw(1) sigbw(1)],[-150 60],'color','r','linestyle','--')
%         plot([sigbw(2) sigbw(2)],[-150 60],'color','r','linestyle','--')
    end
    if isfield(para,'dpdbw')
        performance.dpdbw=dpdbw;
        plot([dpdbw(1) dpdbw(1)],[-150 60],'color','k','linestyle','--')
%         plot([dpdbw(2) dpdbw(2)],[-150 60],'color','k','linestyle','--')
    end
    % HD
    performance.num_HD = num_HD;
    performance.disable_HD_SIG1=disable_HD_SIG1;
    for n=2:num_HD
        if disable_HD_SIG1(n-1)~=1
            HD_ind=f_trans((index_HD_SIG1{n-1}-1),N_fft,fs,nq);
            HD_yrange = 20*log10(mag_all(f_flip(index_HD_SIG1{n-1},nq)));
            performance.HD_ind=HD_ind;
            performance.HD_yrange=HD_yrange;
            plot(HD_ind,HD_yrange,'r');
        end
    end
    if imd_mode==1
        for n=2:num_HD
            if disable_HD_SIG1(n-1)~=1
                imd_mode_HD_ind=f_trans((index_HD_SIG2{n-1}-1),N_fft,fs,nq);
                imd_mode_HD_yrange=20*log10(mag_all(f_flip(index_HD_SIG2{n-1},nq)));
                performance.imd_mode_HD_ind =imd_mode_HD_ind;
                performance.imd_mode_HD_yrange =imd_mode_HD_yrange;
                plot(imd_mode_HD_ind,imd_mode_HD_yrange,'r');
            end
        end
        % IMD
        for n=2:num_IMD
            for k=1:2*(n-1)
                if disable_IMD(n-1,k)~=1
                    num_IMD_ind=f_trans((index_IMD{n-1,k}-1),N_fft,fs,nq);
                    num_IMD_yrange=20*log10(mag_all(f_flip(index_IMD{n-1,k},nq)));
                    performance.num_IMD_ind=num_IMD_ind;
                    performance.num_IMD_yrange=num_IMD_yrange;
                    plot(num_IMD_ind,num_IMD_yrange,'color',[.1 .1 .9]);
                end
            end
        end
    end
    % IL
    performance.num_interleave=num_interleave;
    ...
    if num_interleave~=1
        ...
        for n=1:length(index_center_IL_OS)
            if disable_IL_OS(n)~=1
                os_ind=f_trans((index_IL_OS{n}-1),N_fft,fs,nq);
                os_yrange=20*log10(mag_all(f_flip(index_IL_OS{n},nq)));
                performance.os_ind=os_ind;
                performance.os_yrange=os_yrange;
                plot(os_ind,os_yrange,'g');
            end
        end
        % gain
        ...
        for n=1:length(index_center_IL_GTS_SIG1)
            if disable_IL_GTS_SIG1(n)~=1
                gain_ind=f_trans((index_IL_GTS_SIG1{n}-1),N_fft,fs,nq);
                gain_yrange=20*log10(mag_all(f_flip(index_IL_GTS_SIG1{n},nq)));
                performance.gain_ind=gain_ind;
                performance.gain_yrange=gain_yrange;
                plot(gain_ind,gain_yrange,'m');
            end
        end
        if imd_mode==1
            for n=1:length(index_center_IL_GTS_SIG2)
                if disable_IL_GTS_SIG2(n)~=1
                    gain_imd_mode_ind=f_trans((index_IL_GTS_SIG2{n}-1),N_fft,fs,nq);
                    gain_imd_mode_yrange=20*log10(mag_all(f_flip(index_IL_GTS_SIG2{n},nq)));
                    performance.gain_imd_mode_ind=gain_imd_mode_ind;
                    performance.gain_imd_mode_yrange=gain_imd_mode_yrange;
                    plot(gain_imd_mode_ind,gain_imd_mode_yrange,'m');
                end
            end
        end
    end
    % ref spur
    performance.refclk_ratio=refclk_ratio;
    
    if refclk_ratio~=1
        % 
        for n=1:length(index_center_REF_SPUR_SIG1)
            if disable_REF_SPUR_SIG1(n)~=1
                refclk_ratio_ind=f_trans((index_REF_SPUR_SIG1{n}-1),N_fft,fs,nq);
                refclk_ratio_yrange=20*log10(mag_all(f_flip(index_REF_SPUR_SIG1{n},nq)));
                performance.refclk_ratio_ind=refclk_ratio_ind;
                performance.refclk_ratio_yrange=refclk_ratio_yrange;
                plot(refclk_ratio_ind,refclk_ratio_yrange,'color',[.8 .5 .5]);
            end
        end
        ...
        if imd_mode==1
            for n=1:length(index_center_REF_SPUR_SIG2)
                if disable_REF_SPUR_SIG2(n)~=1
                    refclk_ratio_imd_mode_ind=f_trans((index_REF_SPUR_SIG2{n}-1),N_fft,fs,nq);
                    refclk_ratio_imd_mode_yrange=20*log10(mag_all(f_flip(index_REF_SPUR_SIG2{n},nq)));
                    performance.refclk_ratio_imd_mode_ind=refclk_ratio_imd_mode_ind;
                    performance.refclk_ratio_imd_mode_yrange=refclk_ratio_imd_mode_yrange;
                    plot(refclk_ratio_imd_mode_ind,refclk_ratio_imd_mode_yrange,'color',[.8 .5 .5]);
                end
            end
        end
    end
    % spur
    spur_ind=f_trans((index_spur-1),N_fft,fs,nq);
    spur_yrange=20*log10(mag_all(f_flip(index_spur,nq)));
    performance.spur_ind=spur_ind;
    performance.spur_yrange=spur_yrange;
    plot(spur_ind,spur_yrange,'c');
    % fdet
    if isfield(para,'fdet_array')
        for n=1:length(para.fdet_array)
    ...
    if num_interleave~=1
        if imd_mode==1
            performance_str={performance_str1;performance_str2;performance_str3;performance_str4;performance_str5;performance_str6;performance_str7;performance_str8;performance_str9};
        else
            performance_str={performance_str1;performance_str2;performance_str3;performance_str4;performance_str5;performance_str6;performance_str8;performance_str9};
        end
    else
        if imd_mode==1
